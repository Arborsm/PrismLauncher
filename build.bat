...
if errorlevel 1 (
    call :log_error "CMake未安装，请先安装CMake"
    exit /b 1
)
...
where cl >nul 2>&1
if errorlevel 1 (
    call :log_warning "Visual Studio Build Tools未找到，请确保已安装"
    call :log_info "请运行 'Developer Command Prompt' 或 'x64 Native Tools Command Prompt'"
)
...
cd ..
call :log_success "Electron应用构建完成"
goto :eof
...
echo 示例:
echo   %~nx0              # 构建所有组件
echo   %~nx0 core         # 只构建Core后端
echo   %~nx0 frontend     # 只构建前端
echo   %~nx0 clean        # 清理构建文件
echo.
echo 注意: 请确保在Visual Studio Developer Command Prompt中运行此脚本
goto :eof
